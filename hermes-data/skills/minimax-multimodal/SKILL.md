...
```bash
# Install (global npm install may fail due to permissions — use --prefix)
npm install -g mmx-cli --prefix ~/.local

# Add to PATH (do this in every session or add to ~/.bashrc)
export PATH="$HOME/.local/bin:$PATH"

# Auth — API key location varies by profile; check both paths
# Profile home dir (~/.hermes/profiles/main/home/.hermes/.env) or root ~/.hermes/.env
API_KEY=$(grep MINIMAX_CN_API_KEY ~/.hermes/.env 2>/dev/null | cut -d= -f2)
[ -z "$API_KEY" ] && API_KEY=$(grep MINIMAX_CN_API_KEY ~/.hermes/profiles/main/home/.hermes/.env 2>/dev/null | cut -d= -f2)
...
**Note:** On this system, `~/.hermes/.env` stores the CN API key as `MINIMAX_CN_API_KEY`, not `MINIMAX_API_KEY`.
# Or pass per-call
mmx text chat --api-key sk-xxxxx --message "Hello"
```

Region is auto-detected. Override with `--region global` or `--region cn`.
**Note:** The `.env` file is per-profile under `~/.hermes/profiles/<name>/.env`, not a flat `~/.hermes/.env`. On this system the main profile env is at `~/.hermes/profiles/main/.env`.
...
**Tip:** For best results, describe concept/mood/colors rather than exact symbols or logos. See `references/image-prompts.md` for prompt guidance.
...
1. **Symbol/shape text descriptions unreliable** — AI models cannot reliably generate specific geometric symbols, brand marks, or logo shapes (e.g. `{{>}}`, `<?>`) from text alone. Describe the **concept, colors, and context** instead of the exact shape. For brand-accurate results, describe the visual properties the model CAN generate: flowing ribbon, gradient colors, S-curve/infinity loop shape.

2. **Logo generation without reference** — When asked to generate logos or icons with specific brand shapes (e.g. Microsoft Copilot), describe the visual characteristics rather than symbolic notation. Describe "flowing ribbon with blue-purple gradient, S-curve/infinity loop shape" instead of `{{>}}`.

3. **Image analysis: use `mmx vision describe` for local files** — The built-in `vision_analyze` tool fails on local paths. Always use `mmx vision describe --image <path> --prompt "<question>" --quiet` for local and remote images. This is verified to work reliably across all profiles (main, pet-competitor, sakura).

4. **sensitive content errors** — Vivid descriptions involving weapons, violence, or explicit content trigger `new_sensitive` errors. Reframe with softer language.

5. **API key path** — `~/.hermes/.env` is the canonical location in this workspace; `~/.hermes/profiles/main/home/.hermes/.env` is a secondary path from older setup. Always check both if one fails.
