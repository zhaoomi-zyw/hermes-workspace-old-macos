"""盘中实时 DSA + 低吸条件检查 (2026-09-10)
用法: python intraday_live_dsa.py   # 用腾讯实时价当场算 DSA/MA/回撤
"""
...
HOLD = {"600988": (100, 44.96, 41.81), "000878": (200, 18.31, 17.02)}
...
for s, name in codes.items():
    try:
        ...
        # 用实时价替换当日收盘做 MA
        c = closes[:-1] + [p] if rows[-1][0] < "2026-09-10" or True else closes
        # 简单处理: 若最后一根日线不是今天, 追加实时价
        if rows[-1][0] != "2026-09-10":
        ...
        lo60 = min(h[-60:]) if False else min([float(r[4]) for r in rows[-60:]])
        hi60 = max([float(r[3]) for r in rows[-60:]])
        sc += (p-lo60)/(hi60-lo60)*15 if hi60>lo60 else 7.5
        vma20 = sum(vols[-21:-1])/20
        vtoday = info.get("vol", 0)
        # 盘中量按时间折算(14:03约3.5/4小时)
        vratio = vtoday/vma20/0.88 if vma20 else 1
